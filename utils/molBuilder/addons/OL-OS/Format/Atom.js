...
OpenLayers.Format.Atom = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    parseLocations: function(node) {
		...
		var georssns_bis = this.namespaces.georss10;

        var locations = {components: []};
        var where = this.getElementsByTagNameNS(node, georssns, "where");
		if (!where || where.length == 0)
			where = this.getElementsByTagNameNS(node, georssns_bis, "where");
        if (where && where.length > 0) {
            if (!this.gmlParser) {
                this.initGmlParser();
            }
		...
		if (!point || point.length == 0)
			point = this.getElementsByTagNameNS(node, georssns_bis, "point");
        if (point && point.length > 0) {
		...
		if (!line || line.length == 0)
			line = this.getElementsByTagNameNS(node, georssns_bis, "line");
        if (line && line.length > 0) {
            var coords;
            var p;
            var points;
		...
		if (!polygon || polygon.length == 0)
			polygon = this.getElementsByTagNameNS(node, georssns_bis, "polygon");
        if (polygon && polygon.length > 0) {
            var coords;
            var p;
            var points;
		...
		if (!box || box.length == 0)
			box = this.getElementsByTagNameNS(node, georssns_bis, "box");
        if (box && box.length > 0) {
            for (var i=0, ii=box.length; i<ii; i++) {
                var xyxy = OpenLayers.String.trim(
                            box[i].firstChild.nodeValue
                            ).split(/\s+/);
                if (xyxy.length !=4) {
                    xyxy = OpenLayers.String.trim(
                                box[i].firstChild.nodeValue
                                ).split(/\s*,\s*/);
                }
                components.push(
                    new OpenLayers.Bounds(
                        parseFloat(xyxy[1]), parseFloat(xyxy[0]),
						parseFloat(xyxy[3]), parseFloat(xyxy[2])
                    ).toGeometry()
                );
            }
        }
